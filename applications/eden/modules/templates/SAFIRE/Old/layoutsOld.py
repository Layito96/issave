...
from s3 import *
from s3theme import DIV, NAV, SECTION
    ...
    def layout(item):
        ...
        if item.enabled and item.visible:
            ...
            if item.parent is not None:
                ...
                if item.components:
                    ...
                    if item.get_first(enabled=True, link=True):
                        ...
                        # if item_url == URL(c="event", f="more"):
                            # classei = "fas fa-plus fa-2x ic-menu";
                        # return LI(A(item.label,
                            ...
                            # )
                        return LI(
                            A(IMG(_class="ic-menu", _src=src, _title=title), item.label,
                              _href=_href,
                              _id=item.attr._id
                              ),
                            UL(items,
                    ...
                    if toplevel:
                        ...
                        ############ classe du i############
                        # print(item.url())
                        src = ""  # "/eden/static/themes/SAFIRE/interlinkdesign/img/logo.png"
                        if item_url == URL(c="event", f="incident_report", args=["summary"]):
                            src = "/eden/static/themes/SAFIRE/interlinkdesign/img/calllog.png"
                            title = "Rapport d'Incident"

                        if item_url == URL(c="event", f="incident", args=["summary"]):
                            src = "/eden/static/themes/SAFIRE/interlinkdesign/img/incident.png"
                            title = "Incident"

                        if item_url == URL(c="event", f="scenario"):
                            src = "/eden/static/themes/SAFIRE/interlinkdesign/img/scenarios.png"
                            title = "Scénarios"
                        if item_url == URL(c="default", f="user", args=["logout"]):
                            src = "/eden/static/themes/SAFIRE/interlinkdesign/img/12.png"
                            title = "Déconnection"
                        # return LI(A(item.label,
                            # _href=item_url,
                            # _id=item.attr._id,
                ...
                return DIV(UL(LI(left),
                              _class="top-bar-section sidebar-nav",  # important
                              ),
                           UL(LI(right),
                              _class="top-bar-section sidebar-nav",  # important
                              _style="border-top:50px solid transparent;"
                              ),
                           _id="sidebar-wrapper",
                           )
                # return DIV(UL(
                ...
                # ),
                # DIV(UL(left),
                # _class="top-bar-section", #important
                # ),
                # _class="sidebar-nav",
                # ),
                # _id = "sidebar-wrapper",
