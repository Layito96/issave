                ...
                if incident_id:
                    ...
                    if r.http == "GET":
                        ...
                        def create_onaccept(form):
                            current.db(s3db.event_incident.id == incident_id).update(
                                event_id=form.vars.id)
    ...
    def customise_event_incident_controller(**attr):
        ...
        def custom_prep(r):
            ...
            if method == "create":
                ...
                crud_strings.event_task.msg_list_empty = T(
                    "No Tasks currently registered for this ticket")
                crud_strings.event_organisation.msg_list_empty = T(
                    "No Organizations currently registered in this ticket")
    ...
    def customise_event_asset_resource(r, tablename):
        ...
        if settings.get_incident_label():  # == "Ticket"
            current.response.s3.crud_strings[tablename] = Storage(
                ...
                msg_list_empty=T(
                    "No Equipment currently registered for this ticket"),
            )
        else:
            current.response.s3.crud_strings[tablename] = Storage(
                label_create=T("Add Equipment"),
    ...
    def customise_event_human_resource_resource(r, tablename):
        ...
        if settings.get_incident_label():  # == "Ticket"
            current.response.s3.crud_strings[tablename] = Storage(
                ...
                msg_list_empty=T(
                    "No Persons currently registered for this ticket"),
            )
        else:
            current.response.s3.crud_strings[tablename] = Storage(
                label_create=T("Add Person"),
