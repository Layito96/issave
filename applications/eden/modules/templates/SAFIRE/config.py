...
def config(settings):
    ...
    settings.modules = OrderedDict([
        ...
        ("default", Storage(
            ...
            # All Users (inc Anonymous) can see this module in the default menu & access the controller
            access=None,
            ...
            if name == "incident":
                ...
                if record.event_id or r.method == "event":
                    ...
                    event = A(T("Assign to Event"),
                              ...
                              _class="action-btn btn btn-sm btn-primario"
                              )
                ...
                if link:
                    ...
                    represent = S3Represent(
                        lookup="event_incident", show_link=True)
    ...
    def customise_event_incident_report_resource(r, tablename):
        ...
        current.response.s3.crud_strings[tablename] = Storage(
            label_create=T("Rapport d'incident"),
            title_display=T("Détails rapport d'incident"),
            title_list=T("Rapports d'incidents"),
            title_update=T("Modifier rapport d'incident"),
            label_list_button=T("Liste des rapport d'incidents"),
            label_delete_button=T("Supprimer rapport d'incident"),
            msg_record_created=T("Rapport d'incident ajoutée"),
            msg_record_modified=T("Rapport d'incident mise à jour"),
            msg_record_deleted=T("Rapport d'incident supprimer"),
            msg_list_empty=T("Aucun appel actuellement enregistré"),
        )
    ...
    def customise_event_incident_report_controller(**attr):
        ...
        def custom_prep(r):
            ...
            current.s3db.gis_location.addr_street.label = T(
                "Adresse de la rue ou Détails de l'emplacement")
            from s3 import S3SQLCustomForm
            crud_form = S3SQLCustomForm((T("Description ?"), "name"),
                                        "incident_type_id",
                                        (T("Rapporté par ?"), "reported_by"),
                                        (T("Comment pouvons-nous vous contacter ?"), "contact"),
                                        (T("Où cet incident a-t-il eu lieu ?"),
                                         "location_id"),
                                        (T("Expliquez la situation?"), "description"),
                                        (T("Quels sont vos besoins immédiats?"), "needs"),
                                        )
        ...
        if len(req_args) > 1 and req_args[1] == "assign":
            attr["rheader"] = A(T("Nouvel Incident"),
                                _class="action-btn btn btn-sm btn-primario",
                                _href=URL(c="event", f="incident",
                                          args=["create"],
                                          vars={
                                              "incident_report_id": req_args[0]},
                                          ),
                                )
        else:
            attr["rheader"] = event_rheader
    ...
    def event_incident_create_onaccept(form):
        ...
        if level and int(level) < 3:
            ...
            if incident_type == "Chemical Hazard":
                ...
                current.response.warning = T(
                    "Incident de risque chimique donc niveau augmenté à 3")
        ...
        if organisation_id:
            ...
            if duty:
                current.msg.send_sms_via_api(duty.value,
                                             "Un Incident vous a été Assignée: %s%s" % (settings.get_base_public_url(),
    ...
    def customise_event_incident_resource(r, tablename):
        ...
        f.readable = f.writable = True
        f.label = T("Organisation d’intervention principale")
        if r.method == "plan":
            table.action_plan.label = T("Plan d'action de l'événement")
        else:
            f = table.action_plan
            f.readable = f.writable = False
            ...
            if r.method == "create":
                ...
                if incident_report_id:
                    ...
                    if r.http == "GET":
                        ...
                        table = r.table
                        table.name.default = s3_truncate(
                            incident_report.name, 64)
    ...
    def customise_event_asset_resource(r, tablename):
        ...
        table.item_id.label = T("Type Élément")
        table.asset_id.label = T("Élément Specifique")
        # DateTime
        from gluon import IS_EMPTY_OR
        from gluon import __all__
        # from gluon import IS_LIST_OF_EMAILS
        ...
        for f in (table.start_date, table.end_date):
            ...
            f.represent = lambda dt: S3DateTime.datetime_represent(
                dt, utc=True)
            f.widget = S3CalendarWidget(timepicker=True)

        current.response.s3.crud_strings[tablename] = Storage(
            label_create=T("Ajouter équipement"),
            title_display=T("Détails Équipement"),
            title_list=T("Équipment"),
            title_update=T("Modifier Équipment"),
            label_list_button=T("Liste Équipments"),
            label_delete_button=T("Rétirer l'equipement de cet incident"),
            msg_record_created=T("Équipment ajouter"),
            msg_record_modified=T("Équipment mise à jour"),
            msg_record_deleted=T("Équipment supprimer"),
            msg_list_empty=T("Aucun équipement actuellement enregistré pour cet incident"))
    ...
    def event_human_resource_onaccept(form, create=True):
        ...
        if create:
            ...
            if job_title_id:
                s3db.event_incident_log.insert(incident_id=incident_id,
                                               ...
                                               comments=s3db.event_human_resource.job_title_id.represent(
                                                   job_title_id),
                                               )
        else:
            # Update
        ...
        if pe_id:
            ...
            current.msg.send_by_pe_id(pe_id,
                                      subject="",
                                      message="Un Incident vous a été Assignée: %s%s" %
                                      (settings.get_base_public_url(),
                                       URL(c="event", f="incident",
                                             args=[incident_id, "human_resource", link_id]),
                                       ),
                                      contact_method="SMS"  # A configurer ou commenter pour utiliser EMAIL
                                      )
    # -------------------------------------------------------------------------
    ...
    def customise_event_human_resource_resource(r, tablename):
        ...
        current.response.s3.crud_strings[tablename] = Storage(
            label_create=T("Ajouter une Personne"),
            title_display=T("Détails Personne"),
            title_list=T("Personnel"),
            title_update=T("Modifier Personne"),
            label_list_button=T("Liste du Personnel"),
            label_delete_button=T("Supprimer une personne de cet incident"),
            msg_record_created=T("Personne ajouter"),
            msg_record_modified=T("Personne mise à jour"),
            msg_record_deleted=T("Personne supprimée"),
            msg_list_empty=T("Aucune personne actuellement enregistrée pour cet incident"))
    ...
    def customise_event_scenario_asset_resource(r, tablename):
        ...
        current.response.s3.crud_strings[tablename] = Storage(
            label_create=T("Ajouter d’Équipement"),
            title_display=T("Détails de d’Équipement"),
            title_list=T("d’Équipement"),
            title_update=T("Editer l’équipement "),
            label_list_button=T("Liste Équipement"),
            label_delete_button=T("Retirer l’équipement de cet incident "),
            msg_record_created=T("Équipement ajouté"),
            msg_record_modified=T("Équipement mis à jour"),
            msg_record_deleted=T("Équipement supprimé"),
            msg_list_empty=T("Aucun équipement actuellement enregistré pour cet incident "))
    ...
    def customise_org_organisation_resource(r, tablename):
        ...
        # from s3 import S3SQLCustomForm, S3SQLInlineComponent, S3SQLInlineLink, \
        #                IS_EMPTY_OR, IS_PHONE_NUMBER_MULTI, S3PhoneWidget, s3_phone_represent
        from s3 import S3SQLCustomForm, S3SQLInlineComponent, S3SQLInlineLink, IS_PHONE_NUMBER_MULTI, S3PhoneWidget, s3_phone_represent
        from gluon import IS_EMPTY_OR
    ...
    def project_task_onaccept(form, create=True):
        ...
        if pe_id:
            # Notify Assignee
            message = "Une Tâche vous a été Assignée: %s%s" % \
                (settings.get_base_public_url(),
                 URL(c="event", f="incident",
