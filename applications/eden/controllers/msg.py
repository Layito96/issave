...
def gcm_channel():
    ...
    s3.crud_strings[tablename] = Storage(
        ...
        msg_list_empty=T(
            "No Google Cloud Messaging Settings currently defined"),
...
def sms_modem_channel():
    ...
    try:
        ...
        session.error = T(
            "Python Serial module not available within the running Python - this needs installing to activate the Modem")
...
def twitter_channel():
    ...
    def prep(r):
        ...
        if not (oauth_consumer_key and oauth_consumer_secret):
            session.error = T(
                "You should edit Twitter settings in models/000_config.py")
            return True
        ...
        if r.http == "GET" and r.method in ("create", "update"):
            ...
            try:
                ...
                session.error = T(
                    "Problem connecting to twitter.com - please refresh")
                return True
...
def action_after_save(form):
    ...
    if request.post_vars.get("search_after_save"):
        ...
        session.information = T(
            "The search results should appear shortly - refresh to see them")
...
def twitter_search():
    ...
    try:
        ...
        session.error = T(
            "tweepy module not available within the running Python - this needs installing for non-Tropo Twitter support!")
    ...
    if twitter_settings:
        ...
        try:
            supported_languages = map(lambda x: str(
                x["code"]), twitter_api.supported_languages())
...
def twitter_result():
    ...
    filter_widgets = [
        S3DateFilter("date",
                     ...
                     comment=T(
                         "Filter Tweets by the date they were tweeted on"),
                     ),
...
def person_search(value, type=None):
    ...
    if type == "pr_group" or not type:
        # Check Groups
        query = (groups["name"].lower().like(
            "%" + value + "%")) & (groups.deleted == False)
        rows = db(query).select(groups.pe_id)
        for row in rows:
            items.append({"id": row.pe_id, "name": represent(
                row.pe_id, default_label="")})
    ...
    if type == "pr_person" or not type:
        ...
        # First name
        query = (persons["first_name"].lower().like(
            "%" + value + "%")) & deleted
        ...
        # Middle name
        query = (persons["middle_name"].lower().like(
            "%" + value + "%")) & deleted
        rows = db(query).select(persons.pe_id, cache=s3db.cache)
        for row in rows:
            items.append({"id": row.pe_id, "name": represent(
        ...
        # Last name
        query = (persons["last_name"].lower().like(
            "%" + value + "%")) & deleted
        rows = db(query).select(persons.pe_id, cache=s3db.cache)
        for row in rows:
            items.append({"id": row.pe_id, "name": represent(
...
def readKeyGraph(queryID):
    ...
    write_svg(g.community_fastgreedy().as_clustering().graph,
