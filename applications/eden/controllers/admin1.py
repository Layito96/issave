...
def user():
    ...
    if s3_has_role("ADMIN"):
        ...
        if settings.get_security_policy() < 6:
            ...
            pe_ids = s3db.pr_get_descendants(
                pe_id, entity_types="org_organisation")
            pe_ids.append(pe_id)
    ...
    s3db.configure("auth_user",
                   create_next=URL(c="admin", f="user",
                                   args=["[id]", "roles"]),
                   create_onaccept=lambda form: auth.s3_approve_user(
                       form.vars),
        ...
        if r.interactive and isinstance(output, dict):
            ...
            s3.actions = [{"label": s3_str(UPDATE),
                           "url": URL(c="admin", f="user",
                                      args=["[id]", "update"]),
                           "_class": "action-btn btn btn-sm btn-primary",
                           },
                          {"label": s3_str(T("Roles")),
                           "url": URL(c="admin", f="user",
                                      args=["[id]", "roles"]),
                           "_class": "action-btn btn btn-sm btn-info",
                           },
            ...
            rows = db(query).select(table.id,
                                    table.registration_key)
            s3.dataTableStyleDisabled = s3.dataTableStyleWarning = [
                str(row.id) for row in rows if row.registration_key == "disabled"]
            s3.dataTableStyleAlert = [
                str(row.id) for row in rows if row.registration_key == "pending"]

            # Translate the status values
            values = [{"col": 6, "key": "", "display": s3_str(T("Active"))},
                      {"col": 6, "key": "None",
                          "display": s3_str(T("Active"))},
                      {"col": 6, "key": "pending",
                          "display": s3_str(T("Pending"))},
                      {"col": 6, "key": "disabled",
                          "display": s3_str(T("Disabled"))}
                      ]
...
def errors():
    ...
    def func(p): return os.stat(apath("%s/errors/%s" %
                                      (appname, p), r=request)).st_mtime
...
def create_portable_app(web2py_source, copy_database=False, copy_uploads=False):
    ...
    if copy_uploads:
        ignore = shutil.ignore_patterns(
            "*.db", "*.log", "*.table", "errors", "sessions", "compiled", "cache", ".bzr", "*.pyc")
    else:
        ignore = shutil.ignore_patterns(
            "*.db", "*.log", "*.table", "errors", "sessions", "compiled", "uploads", "cache", ".bzr", "*.pyc")
    ...
    config_template = open(os.path.join(
        appdir, "deployment-templates", "models", "000_config.py"), "r")
    ...
    for base, dirs, files in os.walk(tozip):
        for directory in dirs:
            ...
            zip.write(directory, directory[rootlen:])
        for file in files:
    ...
    def postp(r, output):
        ...
        if opt == "1":
            ...
            if form.accepts(request.vars, session):
                ...
                X = Strings()
                output = X.export_file(
                    code, modlist, [], filetype, all_template_flag)
                return output
