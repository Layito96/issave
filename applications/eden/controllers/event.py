        ...
        if r.interactive:
            ...
            if r.component:
                ...
                #    #s3db.doc_document.location_id.default = r.record.location_id
                    ...
                    s3.crud_strings["dc_response"].label_create = T(
                        "Add Assessment")
                    ...
                    locations = db(ltable.event_id == r.id).select(
                        ltable.location_id)
                    if len(locations) == 1:
            ...
            if r.component:
                if r.component.alias == "assign":
                    if not r.method:
                        r.method = "assign"
                    if r.method == "assign":
                        r.custom_action = s3db.hrm_AssignMethod(
                            component="assign")
                cname = r.component_name
                if cname == "config":
                    ...
                    list_fields = s3db.get_config(
                        "event_sitrep", "list_fields")
                    try:
                    ...
                    if ltable:
                        ...
                        if cname in ("asset", "human_resource"):
                            ...
                            for f in (ltable.start_date, ltable.end_date):
                                ...
                                f.represent = lambda dt: datetime_represent(
                                    dt, utc=True)
