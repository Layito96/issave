...
def define_map(height=None,
    ...
    if poi_resources:
        ...
        for res in poi_resources:
            permit = auth.s3_has_permission(
                "create", res["table"], c=res["c"], f=res["f"])
            if permit:
        ...
        if poi_resources:
            ...
            rows = db(ftable.name.belongs(layers)).select(
                ftable.layer_id, ftable.name)
            layers_lookup = dict()
            for row in rows:
            ...
            script = '''S3.gis.poi_resources=%s''' % json.dumps(
                pois, separators=SEPARATORS)
            s3.js_global.append(script)
            if s3.debug:
    ...
    mauritanie_center = (21.0, -10.0)
    mauritanie_zoom = 6

    map = gis.show_map(height=height,
                       width=width,
                       lat=mauritanie_center[0],
                       lon=mauritanie_center[1],
...
def location():
    ...
    filter_widgets = [
        S3TextFilter(search_fields,
                     label=T("Search"),
                     comment=T(
                         "To search for a location, enter the name. You may use % as wildcard. Press 'Search' without input to list all locations."),
        ...
        if r.interactive and not r.component:
            ...
            if r.method in (None, "list") and r.record is None:
                    ...
                    if record:
                        ...
                        if layer:
                            ...
                            feature_resources.update(tablename="gis_location",
                                                     url="/%s/gis/location.geojson?~.id=%s" % (
                                                         appname, record.id),
                                                     opacity=0.9,
                ...
                _map = gis.show_map(lat = lat,
                                    lon=lon,
                                    zoom=zoom,
                                    bbox={"lon_min": -17.0, "lat_min": 14.5, "lon_max": -4.0, "lat_max": 27.5},
                                    feature_resources=None,
                                    add_feature=False,
                                    add_feature_active=False,
...
def ldata():
    ...
    if not output_level:
        ...
        if translate:
            try:
                id_level = int(locations.as_dict(key="gis_location.id")[
                               location_id]["gis_location"]["level"][1:])
            except:
                return "{}"
        else:
            try:
...
def l0():
    ...
    try:
        ...
        item = current.xml.json_message(
            False, 400, "Need to specify a record ID!")
        ...
        if r.representation == "url":
            ...
            if not r.component:
                ...
                s3.crud_strings["gis_layer_config"] = Storage(
                    ...
                    msg_list_empty=T(
                        "No Layers currently configured in this Profile"),
                )
    ...
    def postp(r, output):
        if r.interactive:
            if r.component_name == "layer_entity":
                ...
                restrict = [str(row.layer_id)
                            for row in rows if not row.enabled]
                s3.actions.append(dict(label=str(T("Enable")),
                                       _class="action-btn  btn btn-sm btn-success",
                                       url=URL(
                                           args=[r.id, "layer_entity", "[id]", "enable"]),
                                       restrict=restrict
                                       ))
                ...
                s3.actions.append(dict(label=str(T("Disable")),
                                       _class="action-btn btn btn-sm btn-danger",
                                       url=URL(
                                           args=[r.id, "layer_entity", "[id]", "disable"]),
                                       restrict=restrict
                                       ))
...
def display_feature():
    ...
    if not s3_has_permission("read", table, feature_id):
        ...
        raise HTTP(401, body=current.xml.json_message(
            False, 401, session.error))
    ...
    if not feature:
        ...
        raise HTTP(404, body=current.xml.json_message(
            False, 404, session.error))
...
def display_features():
    ...
    if ok != 4:
        ...
        raise HTTP(400, body=current.xml.json_message(
            False, 400, session.error))
...
def potlatch2():
    ...
    if opt:
        ...
        session.warning = T(
            "To edit OpenStreetMap, you need to edit the OpenStreetMap settings in your Map Config")
    ...
    try:
        ...
        if allowedHosts and not host in allowedHosts:
            ...
            if method == "POST":
                ...
                headers = {
                    "Content-Type": request["wsgi"].environ["CONTENT_TYPE"]}
