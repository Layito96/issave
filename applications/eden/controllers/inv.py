...
def inv_item():
    ...
    def prep(r):
        ...
        if r.component:
            ...
            s3.crud_strings[tablename].msg_list_empty = T(
                "No Stock currently registered")
            ...
            if r.interactive and \
               ...
               settings.get_inv_direct_stock_edits():
                ...
                if s3.debug:
                    s3.scripts.append(
                        "/%s/static/scripts/S3/s3.inv_item.js" % r.application)
                else:
                    s3.scripts.append(
                        "/%s/static/scripts/S3/s3.inv_item.min.js" % r.application)
                ...
                if r.component_name == "adj_item":
                    ...
                    if adj_status:
                        ...
                        org_site_layout_config(
                            r.record.site_id, s3db.inv_adj_item_bin.layout_id)

                        # Validate Bin Quantities
                        if s3.debug:
                            s3.scripts.append(
                                "/%s/static/scripts/S3/s3.inv_adj_item.js" % r.application)
                        else:
                            s3.scripts.append(
                                "/%s/static/scripts/S3/s3.inv_adj_item.min.js" % r.application)
                ...
                if r.record:
                    ...
                    if "item" in get_vars and "site" in get_vars:
                        ...
                        adj_id = table.insert(adjuster_id=auth.s3_logged_in_person(),
                                              ...
                                              status=0,
                                              category=1,
                                              comments="Adjust %s" % inv_item_table.item_id.represent(
                                                  item_id, show_link=False),
                                              )
    ...
    if viewing:
        ...
        if report == "rel":
            ...
            if settings.get_inv_send_req():
                list_fields.insert(
                    3, (settings.get_inv_req_shortname(), "send_id$req.req_ref"))
            elif settings.get_inv_send_req_ref():
                list_fields.insert(
                    3, (settings.get_inv_req_shortname(), "send_id$req_ref"))
    ...
    def postp(r, output):
        if r.interactive and r.method != "import":
            if not r.component:
                ...
                if s3.debug:
                    s3.scripts.append(
                        "/%s/static/scripts/S3/s3.inv_commit.js" % appname)
                else:
                    s3.scripts.append(
                        "/%s/static/scripts/S3/s3.inv_commit.min.js" % appname)
...
def commit_rheader(r):
    ...
    if r.representation == "html":
        ...
        if record and r.name == "commit":
            ...
            if s3.debug:
                s3.scripts.append(
                    "/%s/static/scripts/S3/s3.inv_req_rheader.js" % appname)
            else:
                s3.scripts.append(
                    "/%s/static/scripts/S3/s3.inv_req_rheader.min.js" % appname)
...
def package():
    ...
    if s3.debug:
        ...
        s3.scripts.append(
            "/%s/static/scripts/S3/s3.inv_package.min.js" % appname)
...
def inv_item_packs():
    ...
    try:
        ...
        raise HTTP(400, current.xml.json_message(
            False, 400, "No value provided!"))
