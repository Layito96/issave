...
def index():
    ...
    def postp(r, output):
        ...
        if r.interactive:
            ...
            s3.actions = [
                dict(label=str(label),
                     _class="action-btn btn-sm btn-primary", url=str(linkto))
            ]
        r.next = None
        return output
    s3.postp = postp
        ...
        if person_id:
            ...
            if r.method in ("create", "create.popup"):
                ...
                filter_widgets = [S3TextFilter(["value",
                                               ...
                                               label=T("Search"),
                                               comment=T(
                                                   "You can search by value or comments."),
                                               ),
        ...
        if r.id:
            if r.method == "join":
                ...
                if user:
                    ...
                    filter_ |= FS(
                        "forum_membership.person_id$pe_id") == user.pe_id
        else:
