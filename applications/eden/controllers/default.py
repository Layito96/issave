...
def index():
    ...
    if page:
        ...
        for template in templates[::-1]:
            package = "applications.%s.modules.templates.%s" % (
                appname, template)
            try:
 ...
 $('#login_form').removeClass('hide')
    ...
    if rss:
        s3.external_stylesheets.append(
            "//www.google.com/uds/solutions/dynamicfeed/gfdynamicfeedcontrol.css")
...
def user():
    ...
    if arg == "login":
        ...
        js_append('''S3.password_min_length=%i''' %
                  settings.get_auth_password_min_length())
        js_append('''i18n.password_min_chars="%s"''' %
                  T("You must enter a minimum of %d characters"))
        ...
        if s3.debug:
            s3.scripts.append(
                "/%s/static/scripts/jquery.pstrength.2.1.0.js" % appname)
        else:
            s3.scripts.append(
                "/%s/static/scripts/jquery.pstrength.2.1.0.min.js" % appname)
...
def person():
    ...
    def auth_profile_method(r, **attr):
        ...
        def onaccept(form): return auth.s3_approve_user(form.vars),
        auth.configure_user_fields()
    ...
    if db_string.find("sqlite") != -1:
        ...
        try:
            ...
            con = MySQLdb.connect(host=settings.database.get("host", "localhost"),
                                  port=settings.database.get(
                                      "port", None) or 3306,
                                  db=settings.database.get(
                                      "database", "sahana"),
                                  user=settings.database.get(
                                      "username", "sahana"),
                                  passwd=settings.database.get(
                                      "password", "password")
                                  )
            cur = con.cursor()
        ...
        try:
            ...
            con = psycopg2.connect(host=settings.database.get("host", "localhost"),
                                   ...
                                   user=settings.database.get(
                                       "username", "sahana"),
                                   password=settings.database.get(
                                       "password", "password")
                                   )
            cur = con.cursor()
            cur.execute("SELECT version()")
